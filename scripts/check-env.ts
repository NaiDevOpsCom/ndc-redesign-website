    ...
    if (stat.isDirectory()) {
      ...
      for (const match of envMatches) {
        ...
        if (!varName.startsWith("VITE_") && !BUILT_IN_VITE_VARS.includes(varName)) {
          console.error(
            `[ERROR] Unsafe environment variable usage found in ${fullPath}: import.meta.env.${varName}`
          );
...
if (totalErrors > 0) {
  if (isHardenedBranch) {
    ...
    console.warn(
      "Hardening Check Failed. Proceeding because this is not a hardened branch (e.g. main/dev)."
    );
  }
}
