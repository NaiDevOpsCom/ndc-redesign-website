...
### Triggers
1.  **Push to `main`**: Automatically triggers a deployment when changes are merged into the main branch.
...
-   `KNOWN_HOSTS`: SSH known hosts for identity verification.
...
Instead of overwriting the existing site, the workflow uses a release-based structure:
...
4.  **Symlink Switch**: The final step atomically updates a symbolic link at `REMOTE_PATH` to point to the new release directory.

### 3. Verification & Rollback
-   **Verification**: After deployment, check the website URL to ensure the new changes are live.
-   **Rollback**: To roll back to a previous version, simply update the symlink manually on the server to point to an older release directory.
...
```
*(Make sure `.secrets` contains values for all required GitHub Secrets)*
